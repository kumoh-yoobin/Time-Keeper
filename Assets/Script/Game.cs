...
public class Game : MonoBehaviour
{
    ...
    private LaserWebSocketClient client = null;
    ...
    public bool getEnablePlayer() {
        return player == currentPlayer;
    }

    public GameObject[] getPlayerPiece() {
        GameObject[] playerPiece = new GameObject[20];
        for (int i = 0; i < playerBlack.Length; i++) {
            playerPiece[i] = playerBlack[i];
        }
        for (int i = 0; i < playerWhite.Length; i++) {
            playerPiece[i + 10] = playerWhite[i];
        }
    ...
    void Start()
    {
        ...
        Create("white_tri_0", 4 , 0), Create("white_splate", 7 , 0),Create(white_laser,"white_laser",0,0),
        Create("white_squre", 0 , 2), Create("white_king", 0 , 3), Create("white_squre_1", 0 , 4), Create("white_tri_4", 0 , 5),
        Create("white_tri_3", 3 , 3), Create("white_tri_2", 3 , 4), Create("white_tri_1", 7 , 6)
        
    };
        playerBlack = new GameObject[] {
        Create("black_tri_0", 3 , 7), Create("black_splate", 0 , 7),Create(black_laser,"black_laser",7,7),
        Create("black_squre", 7 , 3), Create("black_king", 7 , 4), Create("black_squre_1", 7 , 5), Create("black_tri_4", 7 , 2),
    ...
    public GameObject Create(string name, int x, int y)
    {
        ...
        {
            LaserMan laserManScript = laserManObject.GetComponent<LaserMan>();
    ...
    {
        LaserMan Lm = obj.GetComponent<LaserMan>();

        position[Lm.GetXBoard(), Lm.GetYBoard()] = obj;
    }
    ...
    public void sendNextTurn() {
        ...
        if (!this.getEnablePlayer()) return;
